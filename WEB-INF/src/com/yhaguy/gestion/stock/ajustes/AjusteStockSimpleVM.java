...
public class AjusteStockSimpleVM extends SoloViewModel {
	...
	private AjusteStockViewModel dato;
	
	@Init(superclass = true)
	public void init(@ExecutionArgParam(Config.DATO_SOLO_VIEW_MODEL) AjusteStockViewModel dato) {
		this.dato = dato;
	...
	@Command
	public void asignarSigno(@BindingParam("item") AjusteStockDetalleDTO item) {
		if(item.getCantidad() < 0)
			item.setCantidad(item.getCantidad() * -1);
		int signo = this.dato.getDto().isAjustePositivo() ? 1 : -1;
	...
	public AjusteStockViewModel getDato() {
		return dato;
	}

	public void setDato(AjusteStockViewModel dato) {
		this.dato = dato;
	}
}
