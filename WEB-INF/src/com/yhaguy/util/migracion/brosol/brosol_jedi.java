package com.yhaguy.util.migracion.brosol;
...
public class brosol_jedi {


	static Misc m = new Misc();
	

	static String PARTE_FRENOS = "FRENOS";
	
	public static String BROSOLcsv= "./WEB-INF/docs/migracion/articulos/BROSOL.csv";
	...
	public static void migrarDatos() throws Exception {
		...
		// Marcas del Articulo
		Tipo marca_Brosol= rr.getTipoPorSigla(Configuracion.SIGLA_ARTICULO_MARCA_BROSOL) ;
		
		// Partes del Articulo
		Tipo parte_Frenos = rr.getTipoPorSigla(Configuracion.SIGLA_ARTICULO_PARTE_FRENOS);
		...
		while (archivo.ready() == true) {
			...
			if (PARTE.trim().compareTo(PARTE_FRENOS.trim()) == 0) {
				art.setArticuloParte(parte_Frenos);
			} else {
