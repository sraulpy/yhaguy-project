...
public class CinpalMigracion extends CinpalConfiguracion {

	static RegisterDomain rr = RegisterDomain.getInstance();

	public static Tipo CINPAL_MARCA = new Tipo();

	public static Tipo CINPAL_UNIDAD = new Tipo();					
	...
	public static void cargaInicial() throws Exception {
		...
		CINPAL_MARCA.setId((long) 190);
		CINPAL_MARCA.setSigla(Configuracion.SIGLA_ARTICULO_MARCA);
		CINPAL_MARCA.setDescripcion("CINPAL");
		CINPAL_MARCA.setTipoTipo(MARCA_TIPO);

		grabarDB(CINPAL_MARCA);

		// cargar Cinpal como ESTADO TIPO
		// ============================================================================
		...
		CINPAL_UNIDAD.setId((long) 194);
		CINPAL_UNIDAD.setSigla(Configuracion.SIGLA_ARTICULO_UNID_MED);
		CINPAL_UNIDAD.setDescripcion("UNID.");
		CINPAL_UNIDAD.setTipoTipo(UNIDAD_TIPO);

		grabarDB(CINPAL_UNIDAD);
	...
	public static void cargaCinpal() throws Exception {
		...
		while (jedi.ready()) {
			...
			try {
				...
				// ======== codigo original ================
				ResultSet cinpal1 = getConsulta1Cinpal(codigoProducto.trim());
				...
				// ============== modelos aplicacion ==================
				ResultSet cinpal2 = getConsulta2Cinpal(codigoProducto.trim());
...
//						art_cinpal.getArticuloModeloAplicaciones().add(ma);
				...
				if (foto.trim().length() > 0){
					art_cinpal.setUrlImagen(CinpalConfiguracion.URL_IMAGEN+foto);					
				}
				...
				// Cargar Marca
				art_cinpal.setArticuloMarca(CINPAL_MARCA);
				...
				art_cinpal.setArticuloUnidadMedida(CINPAL_UNIDAD);
