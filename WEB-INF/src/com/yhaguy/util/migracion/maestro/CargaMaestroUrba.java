...
import com.yhaguy.util.migracion.maestro.CargaMaestroUrba;




public class CargaMaestroUrba {
	...
	public void urbaOriginal() throws Exception {
		
		
		// ... leer la tabla de Urba
		Class.forName("org.postgresql.Driver");

		String url = "jdbc:postgresql://127.0.0.1:5432/Urba";
		Connection con = DriverManager.getConnection(url, "postgres",
				"postgres");
		Statement rs = con.createStatement();

		String q2Template = "select p.codigoproduto, a.descricaoaplicacaoe as descripcion,  "
				+ " a.complementoaplicacao as complemento, a.complementoaplicacao2 as complemento2,"
				+ " f.descricaofabricante as fabricante "
				+ " from(produto as p  left join produtoaplicacao pa on p.codigoproduto = pa.codigoproduto ) "
		...
		String q1 = "select p.codigoproduto,  MIN(p.numeroproduto)as numeroproduto,"
				...
				+ " from (produto as p  left join referenciacruzada r on "
				+ " p.codigoproduto = r.codigoproduto ) "
		...
		while (rss.next()) {
			...
			String codigoproduto = ("" + rss.getString("codigoproduto")).trim();
			String codigooriginal = getStr(rss.getString("codigooriginal"));
			String numeroproduto = (""+rss.getString("numeroproduto")).trim();
		    String descricaoprodutoe = getStr(rss.getString("descripcion"));
			String arquivofotoproduto = (""+rss.getString("imagen")).trim();
		

		

		   System.out.println(rss.getString("codigoproduto").trim() + " - "  + rss.getString("numeroproduto").trim()+" - "+rss.getString("descripcion").trim()+" - " + rss.getString("imagen").trim()) ;		 
			...
			while (rss2.next()) {
				...
				String complemento2 = getStr(rss2.getString("complemento2"));
				String fabricante = getStr(rss2.getString("fabricante"));
				
				String linea = "["+descripcion +" - "+ complemento +" - "+ complemento2 +" - "+ fabricante+"]\n";
	...
	public static void main(String[] args) throws Exception {
		...
		CargaMaestroUrba ur = new CargaMaestroUrba();
		ur.urbaOriginal();
	}
