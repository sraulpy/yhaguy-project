...
//	private String nombre = "nombre externo";
	private String idMeta = "id-meta";
	private double[] meta = null;
//	private double totalVenta = 0;
//	private double totalContado = 0;
//	private double totalCredito = 0;
	private int metaAlcanzada = 0;

	// private ArrayList<Venta> ventas = new ArrayList<Venta>();
	// private Hashtable<String, Venta> tVentas = new Hashtable<String,
	// Venta>();

	public String toString() {
		String out = super.toString();
		out = this.metaAlcanzada + "(" + (this.getTotalVenta() / 1000000) + ") "
				+ this.idMeta + " " + this.meta;
		return out;
	}


	public void calculaTotales() {
		super.calculaTotales();

		double totalVenta = this.getTotalVenta();
		for (int i = 0; i < meta.length; i++) {
			double m = meta[i];
			if (totalVenta >= m) {
				this.metaAlcanzada = i;
			}
		}
	}
	...
	public String printCabeceraArchivo() {
		...
		String strDevolucion = formatter.format(this.getTotalDevolucion());

		String out = "";
		out += "Vendedor Externo: (" + this.getId() + ") [" + this.getUsuario() + "] "
				+ this.getNombre() + "\n";
		out += "Meta utilizada:" + this.idMeta + "   Posicion: "
				+ this.metaAlcanzada + "  ";
		out += "Total de Ventas: " + strVenta + "      Comision vtas:"
				+ strComision + "	Devolucion: " + strDevolucion;
		return out;
	}
	...
	public String printResumen() {
		...
		String strTotalMes =  strVenta ; // formatter.format(this.getTotalContado());
		...
		// vendedores externos NO cobran pluz por devolucion
		double plusDev = 0; // Cons.calculoDevolucion(this.getTotalVenta(), porDevolucion);
	...
	public static String printResumenCabecera() {
		...
		out2 += m.formato("Meta", 10);
		out2 += m.formato("Pos.", 4);
		out2 += m.formato("Venta Cdo mes", 17, false);
		out2 += m.formato("Venta Cred mes", 17, false);
		out2 += m.formato("Venta Total mes", 17, false);
