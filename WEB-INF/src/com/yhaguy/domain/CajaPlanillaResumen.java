...
public class CajaPlanillaResumen extends Domain {
	...
	/**
	 * @return el total en efectivo ingreso..
	 */
	public double getResumenEfectivoIngreso() {
		double out = 0;
		for (CajaPeriodo planilla : this.planillas) {
			out += (planilla.getTotalEfectivoIngreso());
		}
		return out;
	}
	
	/**
	 * @return el total en efectivo egreso..
	 */
	public double getResumenEfectivoEgreso() {
		double out = 0;
		for (CajaPeriodo planilla : this.planillas) {
			out += (planilla.getTotalEfectivoEgreso());
		}
		return out;
	}
	...
	public List<CajaPeriodo> getPlanillasCobranzas() {
		...
		for (CajaPeriodo planilla : this.planillas) {
			if (planilla.getTipo().equals(CajaPeriodo.TIPO_COBROS) ||
					...
					planilla.getTipo().equals(CajaPeriodo.TIPO_COBROS_MOBILE_INT)) {
				out.add(planilla);
			}
		}
		return out;
